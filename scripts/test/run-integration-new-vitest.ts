...
const IntegrationNewConfigHeader = [
  "# Generated by `pnpm test:integration:new`.",
  "# This file is scoped to one integration-new runner session.",
  "",
].join("\n");
...
async function ensureIntegrationNewConfigPath(input: {
  ...
  if (existingConfigPath !== undefined && existingConfigPath.length > 0) {
    console.info(`[integration-new] using config from MISTLE_CONFIG_PATH: ${existingConfigPath}.`);
    return;
  }
  ...
  await writeFile(
    configPath,
    stringifyTomlConfig({
      header: IntegrationNewConfigHeader,
      configRoot: buildDevelopmentTomlConfig(),
    }),
    "utf8",
  );
  input.environment["MISTLE_CONFIG_PATH"] = configPath;
  console.info(`[integration-new] generated runner config: ${configPath}.`);
}
...
async function main(): Promise<void> {
  ...
  await ensureIntegrationNewConfigPath({
    runnerPoolSession,
