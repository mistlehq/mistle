...
pub(super) fn spawn_claude_code_server_monitor(
    control_handle: ClaudeCodeServerControlHandle,
    shutdown_requested: Arc<AtomicBool>,
) -> JoinHandle<()> {
    thread::spawn(move || {
        run_claude_code_server_monitor(control_handle, shutdown_requested);
    })
}

pub(super) fn run_claude_code_server_monitor(
    control_handle: ClaudeCodeServerControlHandle,
    shutdown_requested: Arc<AtomicBool>,
) {
            ...
            Ok(()) => {
                ...
                if !last_readiness_ok {
                    control_handle
                        .managed_process
                        .supervisor_handle
                        .mark_component_healthy(SupervisedComponent::ClaudeCodeServer);
                }
                control_handle
                    .managed_process
                    .supervisor_handle
                    .record_component_healthcheck(SupervisedComponent::ClaudeCodeServer);
                    ...
                    .replace_component_details(
                        ...
                        claude_code_server_details_with_status(
                            ...
                            if consecutive_readiness_failures
                                >= CLAUDE_CODE_SERVER_POST_START_FAILURE_THRESHOLD
                            {
                                "Unreachable"
                            } else {
                                "Degraded"
                        ...
                        .emit_component_healthcheck_failed(
                            ...
                            &[
                                ...
                                (
                                    "failureThreshold",
                                    Value::String(
                                        CLAUDE_CODE_SERVER_POST_START_FAILURE_THRESHOLD.to_string(),
                                    ),
                                ),
                            ],
                        );
                }
                last_readiness_ok = consecutive_readiness_failures
                    < CLAUDE_CODE_SERVER_POST_START_FAILURE_THRESHOLD;
            }
        }
...
pub(super) fn claude_code_server_details_with_status(
    ...
    let mut details = BTreeMap::from([
        ...
        ("livenessState".to_string(), liveness_state.to_string()),
