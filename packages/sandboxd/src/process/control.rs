...
impl ClaudeCodeServerControlHandle {
    pub fn restart(&self, clock: &dyn Clock, sleeper: &dyn Sleeper) -> Result<(), String> {
        let _restart_guard =
            self.managed_process.restart_lock.lock().map_err(|error| {
                format!("Claude Code server restart lock was poisoned: {error}")
            })?;
        self.managed_process
            .restart_in_progress
            .store(true, Ordering::Relaxed);
        ...
        self.managed_process
            .supervisor_handle
            .mark_component_starting(SupervisedComponent::ClaudeCodeServer);
