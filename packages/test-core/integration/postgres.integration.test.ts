...
const POSTGRES_STACK_TEST_TIMEOUT_MS = 120_000;
...
describe("postgres + pgbouncer integration", () => {
  test(
    "starts stack and exposes reachable direct and pooled endpoints",
    async () => {
    ...
    POSTGRES_STACK_TEST_TIMEOUT_MS,
  );

  test(
    "accepts custom credentials and pooling settings",
    async () => {
      const databaseStack = await startPostgresWithPgBouncer({
  ...
  test(
    "throws when stopping the same stack twice",
    async () => {
      const databaseStack = await startPostgresWithPgBouncer();
      await databaseStack.stop();
