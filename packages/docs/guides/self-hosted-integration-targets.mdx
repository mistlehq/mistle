...
description: Configure integration-targets.provision.json and sync built-in targets into a self-hosted Mistle deployment
---
...
Operators provision those targets in `integration-targets.provision.json`, then sync them into the control-plane database:
...
1. Edit `integration-targets.provision.json`
2. Deploy any referenced environment variables or secrets
3. Run `pnpm --filter @mistle/control-plane-api integration-targets:sync`
...
Use the `openai-default` block from `integration-targets.provision.example.json` as your starting point. That block defines:
