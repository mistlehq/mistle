...
<Note>
  Authorize only the scopes required for the Workspace MCP servers you intend to use, and store the
  service account JSON key carefully.
</Note>
...
3. In Mistle, choose **Service account** when adding a Google Workspace connection.
4. Paste the service account JSON key and enter the Workspace user email Mistle should use.
