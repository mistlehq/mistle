...
For headless Workspace automation, you can connect with a Google Cloud service account that has Workspace domain-wide delegation enabled. This lets Mistle mint short-lived Google access tokens for a delegated Workspace user without an interactive OAuth consent flow.

<Note>
  Domain-wide delegation is powerful. Authorize only the scopes required for the Workspace MCP
  servers you intend to use, store the service account JSON key carefully, and delegate to a
  controlled Workspace user.
</Note>
...
3. In the Google Workspace Admin console, open **Security > API controls > Domain-wide delegation**.
4. Add the service account's OAuth client ID.
5. Authorize the same Google OAuth scopes listed in this guide.
6. Create a JSON key for the service account.
7. In Mistle, choose **Service account with domain-wide delegation** when adding a Google Workspace connection.
8. Paste the service account JSON key and enter the Workspace user email Mistle should delegate to.
