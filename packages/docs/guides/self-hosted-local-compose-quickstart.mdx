---
title: Local Compose Quickstart
description: Run a local Mistle stack with the repository's single-node compose workflow
---

## Overview

For local self-hosted setups, use the single-node compose workflow in `deploy/compose/local`.
It starts the required infrastructure services and one Mistle container for local development and verification.
