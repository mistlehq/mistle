...
as the complete reference for the current TOML shape. During the migration
period, services that read this TOML file must also set:
...
`kv.control_plane` and `kv.data_plane` describe plane-specific Valkey
dependencies.
...
This table covers the operator-facing next TOML keys accepted by
`@mistle/config`.
