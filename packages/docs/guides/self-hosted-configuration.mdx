...
| `kv.*`           | Plane-specific KV runtime configuration                                      |
...
`kv.data_plane` describes the data-plane gateway runtime-state backend.
`kv.control_plane` is available for future control-plane Valkey use, but no
service requires it today.

Use `valkey` for shared or multi-node deployments:
...
Use `memory` only for local single-node deployments. In memory mode, omit
`url` and `key_prefix`:
...
| `kv.data_plane.backend`                                                        | Yes                                              | `valkey` or `memory`. Use `memory` only for local single-node deployments.                |
| `kv.data_plane.url`                                                            | Required when `kv.data_plane.backend = "valkey"` | Data-plane Valkey URL.                                                                    |
| `kv.data_plane.key_prefix`                                                     | Required when `kv.data_plane.backend = "valkey"` | Data-plane Valkey key prefix.                                                             |
