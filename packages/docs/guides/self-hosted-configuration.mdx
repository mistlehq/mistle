...
Stripe billing provisioning is configured under `billing.stripe`. It is disabled
by default. When `enabled = true`, the control plane enqueues Stripe customer
provisioning for newly created organizations and the control-plane worker uses
`secret_key` to create the Stripe customer.
...
| `billing.stripe.enabled`                                                       | No                                         | Enables Stripe customer provisioning for newly created organizations.                                     |
| `billing.stripe.secret_key`                                                    | Yes, when `billing.stripe.enabled = true`  | Stripe secret key used by the control-plane worker to create Stripe customers.                            |
