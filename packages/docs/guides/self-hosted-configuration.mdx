...
`self_service_organization_creation_enabled` controls whether signed-in users
without an organization can create one from the dashboard. It defaults to
`false`. The matching environment override is
`MISTLE_SERVICES_CONTROL_PLANE_API_AUTH_SELF_SERVICE_ORGANIZATION_CREATION_ENABLED`.
...
| `services.control_plane_api.auth.self_service_organization_creation_enabled`   | No                                         | Defaults to `false`. Enable only when users should be able to create organizations from the dashboard.    |
