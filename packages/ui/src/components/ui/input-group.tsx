    ...
    variants: {
      variant: {
        ...
        inline: cn(
          ...
          "[&_[data-slot=input-group-addon]]:pl-0.5 [&_[data-slot=input-group-addon]]:pr-2 [&_[data-slot=input-group-addon]]:text-inherit [&_[data-slot=input-group-control]::placeholder]:text-current data-[active=true]:[&_[data-slot=input-group-addon]]:text-muted-foreground data-[active=true]:[&_[data-slot=input-group-control]::placeholder]:text-muted-foreground",
        ),
      },
    },
    defaultVariants: {
...
const inputGroupInputVariants = cva("w-full min-w-0 outline-none", {
  variants: {
    variant: {
      default:
        "flex-1 h-full rounded-none border-0 bg-transparent px-0 text-base shadow-none ring-0 md:text-sm focus-visible:ring-0 focus-visible:ring-offset-0 aria-invalid:ring-0 dark:bg-transparent disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50",
      inline:
        "flex-1 h-full rounded-none border-0 bg-transparent px-0 text-base shadow-none ring-0 md:text-sm focus-visible:ring-0 focus-visible:ring-offset-0 aria-invalid:ring-0 dark:bg-transparent disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50",
    },
  },
...
function InputGroupAddon({
  ...
  function focusNearestInput(target: EventTarget & HTMLDivElement): void {
    const control = target.parentElement?.querySelector("input, textarea");
  ...
  return (
    <div
      ...
      onClick={(e) => {
        ...
        focusNearestInput(e.currentTarget);
      }}
      onKeyDown={(e) => {
...
function InputGroupInput({
  ...
  variant = "default",
  ...props
}: React.ComponentProps<"input"> & VariantProps<typeof inputGroupInputVariants>) {
  return (
    <Input
