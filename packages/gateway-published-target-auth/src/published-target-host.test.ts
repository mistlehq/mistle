...
describe("@mistle/gateway-published-target-auth published target host", () => {
  ...
  it("derives and parses a published app host", () => {
    const host = derivePublishedTargetHost({
      baseDomain: "example.com",
      sandboxInstanceId: "sbi_app_001",
      target: {
        kind: "app",
        appKey: "ide",
      },
    ...
    expect(host).toBe("a-ide--sbi-app-001.example.com");
    ...
    expect(
      ...
      baseDomain: "example.com",
      host: "a-ide--sbi-app-001.example.com",
      sandboxInstanceId: "sbi_app_001",
      target: {
        kind: "app",
        appKey: "ide",
  ...
  it("rejects invalid port labels", async () => {
    ...
    expect(error.code).toBe(PublishedTargetHostErrorCode.TARGET_PORT_INVALID);
  });
  ...
  it("rejects non-canonical base domains", async () => {
    ...
    expect(error.code).toBe(PublishedTargetHostErrorCode.HOST_BASE_DOMAIN_MISMATCH);
  });
