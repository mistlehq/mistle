import { createSecretKey } from "node:crypto";

import { SignJWT, errors as JoseErrors, jwtVerify } from "jose";

const AllowedConnectionTokenAlgorithms = ["HS256"];
const JwtSecretEncoder = new TextEncoder();

export type ConnectionTokenConfig = {
  connectionTokenSecret: string;
  tokenIssuer: string;
  tokenAudience: string;
};

export type VerifiedConnectionToken = {
  jti: string;
  sandboxInstanceId: string;
};

export const ConnectionTokenErrorCode = {
  TOKEN_REQUIRED: "TOKEN_REQUIRED",
  JTI_REQUIRED: "JTI_REQUIRED",
  SANDBOX_INSTANCE_ID_REQUIRED: "SANDBOX_INSTANCE_ID_REQUIRED",
  INVALID_TTL_SECONDS: "INVALID_TTL_SECONDS",
  TOKEN_EXPIRED: "TOKEN_EXPIRED",
  TOKEN_INVALID_ISSUER: "TOKEN_INVALID_ISSUER",
  TOKEN_INVALID_AUDIENCE: "TOKEN_INVALID_AUDIENCE",
  TOKEN_INVALID_CLAIMS: "TOKEN_INVALID_CLAIMS",
  TOKEN_VERIFICATION_FAILED: "TOKEN_VERIFICATION_FAILED",
  TOKEN_SIGNING_FAILED: "TOKEN_SIGNING_FAILED",
} as const;

export type ConnectionTokenErrorCode =
  (typeof ConnectionTokenErrorCode)[keyof typeof ConnectionTokenErrorCode];

type ConnectionTokenErrorInput = {
  code: ConnectionTokenErrorCode;
  message: string;
  cause?: unknown;
};

export class ConnectionTokenError extends Error {
  readonly code: ConnectionTokenErrorCode;

  constructor(input: ConnectionTokenErrorInput) {
    super(input.message, { cause: input.cause });
    this.name = "ConnectionTokenError";
    this.code = input.code;
  }
}

function toNonEmptyString(value: string | undefined): string | undefined {
  const normalized = value?.trim();
  if (normalized === undefined || normalized.length === 0) {
    return undefined;
  }

  return normalized;
}

function toSecretKey(secret: string): ReturnType<typeof createSecretKey> {
  return createSecretKey(JwtSecretEncoder.encode(secret));
}

function mapClaimValidationErrorCode(
  error: JoseErrors.JWTClaimValidationFailed,
): ConnectionTokenErrorCode {
  if (error.claim === "iss") {
    return ConnectionTokenErrorCode.TOKEN_INVALID_ISSUER;
  }

  if (error.claim === "aud") {
    return ConnectionTokenErrorCode.TOKEN_INVALID_AUDIENCE;
  }

  return ConnectionTokenErrorCode.TOKEN_INVALID_CLAIMS;
}

export async function mintConnectionToken(input: {
  config: ConnectionTokenConfig;
  jti: string;
  sandboxInstanceId: string;
  ttlSeconds: number;
}): Promise<string> {
  const normalizedJti = toNonEmptyString(input.jti);
  if (normalizedJti === undefined) {
    throw new ConnectionTokenError({
      code: ConnectionTokenErrorCode.JTI_REQUIRED,
      message: "Connection token jti claim is required.",
    });
  }
  const normalizedSandboxInstanceId = toNonEmptyString(input.sandboxInstanceId);
  if (normalizedSandboxInstanceId === undefined) {
    throw new ConnectionTokenError({
      code: ConnectionTokenErrorCode.SANDBOX_INSTANCE_ID_REQUIRED,
      message: "Connection token sandboxInstanceId claim is required.",
    });
  }

  if (!Number.isInteger(input.ttlSeconds) || input.ttlSeconds < 1) {
    throw new ConnectionTokenError({
      code: ConnectionTokenErrorCode.INVALID_TTL_SECONDS,
      message: "Connection token ttlSeconds must be an integer greater than or equal to 1.",
    });
  }

  const nowEpochSeconds = Math.floor(Date.now() / 1000);

  try {
    return await new SignJWT({
      sandboxInstanceId: normalizedSandboxInstanceId,
    })
      .setProtectedHeader({ alg: "HS256" })
      .setJti(normalizedJti)
      .setIssuer(input.config.tokenIssuer)
      .setAudience(input.config.tokenAudience)
      .setIssuedAt(nowEpochSeconds)
      .setExpirationTime(nowEpochSeconds + input.ttlSeconds)
      .sign(toSecretKey(input.config.connectionTokenSecret));
  } catch (error) {
    throw new ConnectionTokenError({
      code: ConnectionTokenErrorCode.TOKEN_SIGNING_FAILED,
      message: "Failed to sign connection token.",
      cause: error,
    });
  }
}

export async function verifyConnectionToken(input: {
  config: ConnectionTokenConfig;
  token: string;
}): Promise<VerifiedConnectionToken> {
  const normalizedToken = toNonEmptyString(input.token);
  if (normalizedToken === undefined) {
    throw new ConnectionTokenError({
      code: ConnectionTokenErrorCode.TOKEN_REQUIRED,
      message: "Connection token is required.",
    });
  }

  let payloadJti: string | undefined;
  let payloadSandboxInstanceId: string | undefined;
  try {
    const verificationResult = await jwtVerify(
      normalizedToken,
      toSecretKey(input.config.connectionTokenSecret),
      {
        algorithms: AllowedConnectionTokenAlgorithms,
        issuer: input.config.tokenIssuer,
        audience: input.config.tokenAudience,
      },
    );
    payloadJti = verificationResult.payload.jti;
    if (typeof verificationResult.payload.sandboxInstanceId === "string") {
      payloadSandboxInstanceId = verificationResult.payload.sandboxInstanceId;
    }
  } catch (error) {
    if (error instanceof JoseErrors.JWTExpired) {
      throw new ConnectionTokenError({
        code: ConnectionTokenErrorCode.TOKEN_EXPIRED,
        message: "Connection token is expired.",
        cause: error,
      });
    }

    if (error instanceof JoseErrors.JWTClaimValidationFailed) {
      throw new ConnectionTokenError({
        code: mapClaimValidationErrorCode(error),
        message: "Connection token claim validation failed.",
        cause: error,
      });
    }

    if (error instanceof JoseErrors.JOSEError) {
      throw new ConnectionTokenError({
        code: ConnectionTokenErrorCode.TOKEN_VERIFICATION_FAILED,
        message: "Connection token verification failed.",
        cause: error,
      });
    }

    throw new ConnectionTokenError({
      code: ConnectionTokenErrorCode.TOKEN_VERIFICATION_FAILED,
      message: "Connection token verification failed with unexpected error.",
      cause: error,
    });
  }

  const normalizedJti = toNonEmptyString(payloadJti);
  if (normalizedJti === undefined) {
    throw new ConnectionTokenError({
      code: ConnectionTokenErrorCode.JTI_REQUIRED,
      message: "Connection token jti claim is required.",
    });
  }
  const normalizedSandboxInstanceId = toNonEmptyString(payloadSandboxInstanceId);
  if (normalizedSandboxInstanceId === undefined) {
    throw new ConnectionTokenError({
      code: ConnectionTokenErrorCode.SANDBOX_INSTANCE_ID_REQUIRED,
      message: "Connection token sandboxInstanceId claim is required.",
    });
  }

  return {
    jti: normalizedJti,
    sandboxInstanceId: normalizedSandboxInstanceId,
  };
}
