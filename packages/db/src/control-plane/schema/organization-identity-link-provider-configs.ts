...
export function defineOrganizationIdentityLinkProviderConfigs(schema: PgSchema) {
  return schema.table(
    ...
    (table) => [
      ...
      index("org_identity_link_provider_cfgs_org_status_idx").on(
        table.organizationId,
        table.status,
      ),
...
export const organizationIdentityLinkProviderConfigs =
  defineOrganizationIdentityLinkProviderConfigs(controlPlaneSchema);
