...
- `inspect({ id })` returns Docker-backed container metadata for the runtime id without attaching to the container.
...
- `operation`: identifies failing operation (`pull_image`, `create_container`, `apply_startup`, etc.)
