...
- `inspect({ id })` returns E2B-backed sandbox metadata for the sandbox id without connecting to the runtime, including both `templateId` and a stable `templateAlias`.
- The provider persists the derived template alias in sandbox metadata at creation time so inspection can recover the stable alias across pause/resume.
