...
function installSandboxBaseCommon(image: Image): Image {
  return image
    ...
    .run(
      [
        "apt-get update",
        "apt-get install -y --no-install-recommends bash ca-certificates coreutils curl dbus findutils file fuse gawk git grep iproute2 jq kmod less libatomic1 linux-modules-$(uname -r) lsof nftables procps psmisc ripgrep rsync sed systemd systemd-sysv tar unzip zip",
