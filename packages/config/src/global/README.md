...
| `sandbox.storage.e2b`             | `"archil"`                      | Durable storage backend used when `sandbox.provider = "e2b"`.    | None    | `[global.sandbox.storage].e2b`              | `MISTLE_GLOBAL_SANDBOX_STORAGE_E2B`              |
| `sandbox.storage.docker`          | `"docker_volume"`               | Durable storage backend used when `sandbox.provider = "docker"`. | None    | `[global.sandbox.storage].docker`           | `MISTLE_GLOBAL_SANDBOX_STORAGE_DOCKER`           |
| `sandbox.defaultBaseImage`        | `string`                        | Shared canonical OCI base image used when starting sandboxes.    | None    | `[global.sandbox].default_base_image`       | `MISTLE_GLOBAL_SANDBOX_DEFAULT_BASE_IMAGE`       |
