...
function applyDescriptor(
  ...
  const surface = descriptor.surface ?? "legacy";
  ...
  if (existingValue !== undefined && !valuesAreEqual(existingValue, value)) {
    if (appliedOverride?.surface === "new" && surface === "legacy") {
      return root;
    }

    if (appliedOverride?.surface === "legacy" && surface === "new") {
...
const LegacyRootEnvDescriptors: readonly RootEnvDescriptor[] = [
  {
    envVar: "NODE_ENV",
    path: ["global", "env"],
...
const RootEnvDescriptors = [
  ...NewRootEnvDescriptors,
  ...LegacyRootEnvDescriptors,
] satisfies readonly RootEnvDescriptor[];
