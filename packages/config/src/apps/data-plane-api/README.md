...
| `workflow.databaseUrl`        | `string`              | Direct Postgres URL used by OpenWorkflow producer in data-plane.       | None    | `MISTLE_POSTGRES_DATA_PLANE_DIRECT_URL`                                |
...
| `workflow.databasePoolMax`    | `number` (`>=1`)      | Maximum direct Postgres connections for the API OpenWorkflow pools.    | None    | `MISTLE_SERVICES_DATA_PLANE_API_WORKFLOW_DATABASE_POOL_MAX` (`Number`) |
...
| `sandbox.docker.enabled`      | `boolean`             | Whether Docker sandbox inspection config is available.                 | None    | `MISTLE_SANDBOX_DOCKER_ENABLED`                                        |
| `sandbox.docker.socketPath`   | `string`              | Docker socket path used for sandbox inspection when Docker is enabled. | None    | `MISTLE_SANDBOX_DOCKER_SOCKET_PATH`                                    |
