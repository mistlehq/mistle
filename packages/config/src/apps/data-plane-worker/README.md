...
| `sandboxStorage.dockerVolume.namePrefix` | `string` (optional) | Optional prefix applied to new Docker volume names.                | None      | `[apps.data_plane_worker.sandbox_storage.docker_volume].name_prefix` | `MISTLE_APPS_DATA_PLANE_WORKER_SANDBOX_STORAGE_DOCKER_VOLUME_NAME_PREFIX`     |
...
- `sandboxStorage.dockerVolume` is only used when `global.sandbox.storage.docker = "docker_volume"`.
