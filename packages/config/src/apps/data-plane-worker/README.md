...
| `tunnel.gatewayWsUrl`               | `string`              | WebSocket URL exposed by data-plane-gateway for sandbox tunnels.  | None    | `[apps.data_plane_worker.tunnel].gateway_ws_url`              | `MISTLE_APPS_DATA_PLANE_WORKER_TUNNEL_GATEWAY_WS_URL`                         |
...
| `sandbox.modal.tokenId`             | `string`              | Modal token id used when `global.sandbox.provider` is modal.      | None    | `[apps.data_plane_worker.sandbox.modal].token_id`             | `MISTLE_APPS_DATA_PLANE_WORKER_SANDBOX_MODAL_TOKEN_ID`                        |
| `sandbox.modal.tokenSecret`         | `string`              | Modal token secret used when `global.sandbox.provider` is modal.  | None    | `[apps.data_plane_worker.sandbox.modal].token_secret`         | `MISTLE_APPS_DATA_PLANE_WORKER_SANDBOX_MODAL_TOKEN_SECRET`                    |
...
| `sandbox.docker.socketPath`         | `string`              | Docker socket path used when `global.sandbox.provider` is docker. | None    | `[apps.data_plane_worker.sandbox.docker].socket_path`         | `MISTLE_APPS_DATA_PLANE_WORKER_SANDBOX_DOCKER_SOCKET_PATH`                    |
