...
| -------------------------- | --------------------- | --------------------------------------------------------------------- | ------- | ---------------------------------------------------------------------------- |
| `database.url`             | `string`              | Runtime Postgres connection URL for control-plane worker app data.    | None    | `MISTLE_POSTGRES_CONTROL_PLANE_POOLED_URL`                                   |
| `workflow.databaseUrl`     | `string`              | Direct Postgres URL used by OpenWorkflow backend in the worker.       | None    | `MISTLE_POSTGRES_CONTROL_PLANE_DIRECT_URL`                                   |
...
| `workflow.databasePoolMax` | `number` (`>=1`)      | Maximum direct Postgres connections for the worker OpenWorkflow pool. | None    | `MISTLE_SERVICES_CONTROL_PLANE_WORKER_WORKFLOW_DATABASE_POOL_MAX` (`Number`) |
...
| `billing.stripe.enabled`   | `boolean`             | Whether Stripe customer provisioning workflows can call Stripe.       | `false` | `MISTLE_BILLING_STRIPE_ENABLED`                                              |
| `billing.stripe.secretKey` | `string`              | Stripe secret key used by customer provisioning workflows.            | None    | `MISTLE_BILLING_STRIPE_SECRET_KEY`                                           |
