...
| `controlPlaneApi.baseUrl`              | `string`               | Internal base URL for gateway calls into `control-plane-api`. | None     | `[apps.data_plane_gateway.control_plane_api].base_url`              | `MISTLE_APPS_DATA_PLANE_GATEWAY_CONTROL_PLANE_API_BASE_URL`              |
| `lifecycle.idleTimeoutMs`              | `number` (`>0`)        | Sandbox idle timeout before the gateway begins cleanup.       | `300000` | `[apps.data_plane_gateway.lifecycle].idle_timeout_ms`               | `MISTLE_APPS_DATA_PLANE_GATEWAY_LIFECYCLE_IDLE_TIMEOUT_MS`               |
| `lifecycle.bootstrapDisconnectGraceMs` | `number` (`>0`)        | Grace window after bootstrap disconnect before cleanup.       | `60000`  | `[apps.data_plane_gateway.lifecycle].bootstrap_disconnect_grace_ms` | `MISTLE_APPS_DATA_PLANE_GATEWAY_LIFECYCLE_BOOTSTRAP_DISCONNECT_GRACE_MS` |
