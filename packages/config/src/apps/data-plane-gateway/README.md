...
| `health.websocketPingIntervalMs`     | `number`               | Websocket health-check ping interval in milliseconds.         | `10000` | `MISTLE_SERVICES_DATA_PLANE_GATEWAY_HEALTH_WEBSOCKET_PING_INTERVAL_MS`      |
| `health.websocketPongTimeoutMs`      | `number`               | Websocket health-check pong timeout in milliseconds.          | `10000` | `MISTLE_SERVICES_DATA_PLANE_GATEWAY_HEALTH_WEBSOCKET_PONG_TIMEOUT_MS`       |
| `portAccess.authorizationTimeoutMs`  | `number`               | Sandbox target authorization timeout in milliseconds.         | `5000`  | `MISTLE_SERVICES_DATA_PLANE_GATEWAY_PORT_ACCESS_AUTHORIZATION_TIMEOUT_MS`   |
| `tunnel.maxActiveBindingsPerSandbox` | `number`               | Active interactive stream binding cap per sandbox tunnel.     | `32`    | `MISTLE_SERVICES_DATA_PLANE_GATEWAY_TUNNEL_MAX_ACTIVE_BINDINGS_PER_SANDBOX` |
