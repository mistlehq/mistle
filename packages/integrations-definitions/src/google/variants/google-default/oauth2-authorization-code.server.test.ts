...
import {
  buildGoogleAuthorizationCodeExchangeRequestBody,
  buildGoogleAuthorizationUrl,
  buildGoogleRefreshRequestBody,
  classifyGoogleRefreshFailure,
  createGoogleRefreshTransportFailure,
  GoogleOAuth2AuthorizationCodeCapability,
  resolveGoogleAccessTokenExpiresAt,
  resolveGoogleAuthorizationCodeOrThrow,
  resolveGoogleCompleteGrantResult,
  resolveGoogleProviderState,
  resolveGoogleRefreshResult,
} from "./oauth2-authorization-code.server.js";

describe("Google OAuth authorization code support", () => {
  it("builds a Google authorization URL with caller-provided scopes and offline access", () => {
  ...
  it("starts authorization through the capability and stores scopes in provider state", async () => {
    const result = await GoogleOAuth2AuthorizationCodeCapability.startAuthorization({
  ...
  it("requires PKCE when starting authorization", () => {
    expect(() =>
      GoogleOAuth2AuthorizationCodeCapability.startAuthorization({
        organizationId: "org_123",
        targetKey: "google-default",
        target: {
          familyId: "google",
    ...
    ).toThrow("Google OAuth authorization requires PKCE.");
  });

  it("resolves callback authorization codes and provider errors", () => {
    expect(resolveGoogleAuthorizationCodeOrThrow(new URLSearchParams("code=code_123"))).toBe(
      "code_123",
    );
    ...
    ).toThrow("Google OAuth authorization failed with error 'access_denied': Denied");

    expect(() =>
      resolveGoogleAuthorizationCodeOrThrow(new URLSearchParams("state=state_abc")),
    ).toThrow("Google OAuth callback query must include `code`.");
  });
  ...
  it("resolves grant and refresh results with token expiry and scope metadata", () => {
    ...
    const requestedScopes = ["https://www.googleapis.com/auth/analytics.readonly"];
  ...
  it("accepts non-URL OAuth scope tokens in callback provider state", () => {
  ...
  it("classifies refresh failures from Google OAuth status and error bodies", () => {
    ...
    expect(
      ...
      classification:
        IntegrationOAuth2AuthorizationCodeRefreshAccessTokenErrorClassifications.TEMPORARY,
      message: "Google access token refresh failed with status 200.",
    });
  });

  it("wraps refresh transport errors as temporary refresh failures", () => {
    const error = createGoogleRefreshTransportFailure({
    ...
    expect(error.message).toBe(
      "Google OAuth refresh request failed before a response was received: socket closed",
    );
  });
});
