...
| Start Sandbox Profile Instance | `StartSandboxProfileInstanceWorkflowSpec` | `control-plane.sandbox-instances.start-profile-instance` | `{ organizationId: string; sandboxProfileId: string; sandboxProfileVersion: number; startedBy: { kind: string; id: string }; source: string }` | `{ workflowRunId: string; sandboxInstanceId: string; providerSandboxId: string }` | Resolves profile-version configuration and starts a sandbox via data-plane API. |
