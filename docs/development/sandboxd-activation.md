...
`snapshot` activates a one-off snapshot materialization sandbox. Sandboxd applies the published runtime plan and setup script for image capture, then the caller captures the provider image and destroys the sandbox.
