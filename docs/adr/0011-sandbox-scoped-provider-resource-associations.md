...
Association matching evaluates captured associated resource event routing, including any association event parameter rules, before self-authorship. Self-authorship is only evaluated for events that would otherwise be eligible for an Association delivery, keeping routing-disabled or filter-excluded events distinct from self-authored suppression.
