...
Designer `dashboard_control.request_user_input` custom responses use the same dynamic-tool response wrapper as structured answers: the dashboard returns a successful tool response whose text content is JSON containing `customResponse.text`. Designer managed instructions should treat that JSON as the user's custom response to the pending decision.

Designer `dashboard_control.request_user_input` should not define a separate inline custom-answer field. The pending request's composer custom response is the single path for unlisted answers or changes in direction, which avoids presenting two custom-answer surfaces for the same decision.
