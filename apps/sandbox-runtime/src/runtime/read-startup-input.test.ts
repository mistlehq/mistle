...
describe("readStartupInput", () => {
  ...
  it("fails when stdin is empty", async () => {
    ...
    ).rejects.toThrow("startup input is empty");
  });
  ...
  it("fails when startup input has trailing json content in the same stream chunk", async () => {
    await expect(
      readStartupInput({
        reader: createReader(`${ValidStartupInputJson} {"unexpected":true}`),
        maxBytes: 4096,
      }),
    ).rejects.toThrow("startup input must be valid json: unexpected trailing JSON content");
  });
});
