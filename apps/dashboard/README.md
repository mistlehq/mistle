...
- `MISTLE_SERVICES_DASHBOARD_CONTROL_PLANE_API_ORIGIN` can provide the control-plane API origin directly when no TOML config file is present.
...
- Docker image builds can set `MISTLE_SERVICES_DASHBOARD_CONTROL_PLANE_API_ORIGIN` directly when no config file is present, or override the dashboard config file with `MISTLE_CONFIG_PATH`.
- Override config file path by setting:
  - `MISTLE_CONFIG_PATH`
- Override the control-plane API origin directly by setting:
  - `MISTLE_SERVICES_DASHBOARD_CONTROL_PLANE_API_ORIGIN`
- PostHog product analytics is optional and disabled by default:
  - `services.dashboard.posthog.enabled = false`
  - set `services.dashboard.posthog.enabled = true` with `project_api_key` and `host` to enable it
  - direct env overrides are available as `MISTLE_SERVICES_DASHBOARD_POSTHOG_ENABLED`, `MISTLE_SERVICES_DASHBOARD_POSTHOG_PROJECT_API_KEY`, and `MISTLE_SERVICES_DASHBOARD_POSTHOG_HOST`
- Required key:
