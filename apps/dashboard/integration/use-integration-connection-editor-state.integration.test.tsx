...
import type { ManagedWebhookSetupResult } from "../src/features/integrations/integrations-service-shared.js";
...
} from "../src/test-support/query-client.js";
...
function multiMethodGitHubCreateEditorInput() {
  return {
    mode: "create" as const,
    methods: [
      {
...
function legacyOpenAiFormUpdateEditorInput() {
  return {
    mode: "update" as const,
    connectionConfig: {
...
describe("useIntegrationConnectionEditorState", () => {
    ...
    const { result } = renderHook(
      () =>
        useIntegrationConnectionEditorState({
          initialEditorInput: multiMethodGitHubCreateEditorInput(),
  ...
  it("submits form updates without secret when the secret is whitespace", async () => {
    server.setHandler((request) => {
      if (
        request.method === "PUT" &&
        request.pathname === "/v1/integration/connections/icn_123/form"
      ) {
        return {
          status: 200,
          body: {
    ...
    const { result } = renderHook(
      () =>
        useIntegrationConnectionEditorState({
          initialEditorInput: legacyOpenAiFormUpdateEditorInput(),
          queryKey: ["integration-directory"],
        }),
      {
        wrapper: createWrapper(queryClient),
    ...
    await waitFor(() => {
      expect(server.requests).toEqual([
        {
          method: "PUT",
          pathname: "/v1/integration/connections/icn_123/form",
          body: {
            displayName: "Renamed connection",
            config: {},
          },
