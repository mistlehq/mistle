...
import { compactMap, readMembersArray } from "./members-records.js";

describe("members records helpers", () => {
  it("returns null when reading arrays from non-array values", () => {
    expect(readMembersArray(null)).toBeNull();
    expect(readMembersArray("members")).toBeNull();
  });

  it("returns arrays unchanged when value is an array", () => {
    const value = ["owner", "member"];
    expect(readMembersArray(value)).toEqual(value);
  });
