...
type GroupedCheckboxesGroup = {
  label: string;
  values: readonly string[];
};

function GroupedCheckboxesWidget(
  props: WidgetProps<JsonObject, RJSFSchema, SchemaFormContext>,
): React.JSX.Element {
  const {
    autofocus = false,
    ...
    onChange,
    onFocus,
    options: { emptyValue, enumDisabled, enumOptions },
    readonly,
  } = props;
  const values = Array.isArray(props.value) ? props.value : [props.value];
  const groups = resolveGroupedCheckboxesGroups(props);
  const enumOptionsByValue = resolveEnumOptionsByValue(enumOptions);

  return (
    <div data-slot="grouped-checkbox-group" className="flex flex-col gap-4" id={id}>
      {groups.map((group, groupIndex) => (
        <fieldset className="flex flex-col gap-2" key={group.label}>
          <legend className="text-muted-foreground text-xs font-medium">{group.label}</legend>
          <div className="flex flex-col gap-3">
            {group.values.map((value, valueIndex) => {
              const option = enumOptionsByValue.get(value);
              if (option === undefined) {
                throw new Error(`Grouped checkboxes option '${value}' is not defined.`);
              }

              const checked = enumOptionsIsSelected(option.value, values);
              const itemDisabled =
                Array.isArray(enumDisabled) && enumDisabled.includes(option.value);
              const itemId = optionId(id, option.index);
              const describedBy = ariaDescribedByIds(id);
              const optionLabel = resolveCheckboxOptionLabel(props, {
                label: String(option.label),
                value: option.value,
              ...
              return (
                ...
                >
                  <Checkbox
                    aria-describedby={describedBy}
                    autoFocus={autofocus && groupIndex === 0 && valueIndex === 0}
                    checked={checked}
                    disabled={disabled || itemDisabled || readonly}
                    id={itemId}
                    name={htmlName ?? id}
                    onBlur={() => {
                      onBlur(
                        id,
                        enumOptionsValueForIndex(String(option.index), enumOptions, emptyValue),
                      );
                    }}
                    onCheckedChange={(nextChecked) => {
                      if (nextChecked) {
                        onChange(enumOptionsSelectValue(option.index, values, enumOptions));
                        return;
                      }

                      onChange(enumOptionsDeselectValue(option.index, values, enumOptions));
                    }}
                    onFocus={() => {
                      onFocus(
                        id,
...
function resolveGroupedCheckboxesGroups(
  props: WidgetProps<JsonObject, RJSFSchema, SchemaFormContext>,
): readonly GroupedCheckboxesGroup[] {
  const groups = props.options.groups;
  if (!Array.isArray(groups)) {
    throw new Error("Grouped checkboxes widget requires a groups option.");
  }

  const enumValues = resolveEnumValues(props.options.enumOptions);
  const groupedValues = new Set<string>();
  const resolvedGroups: GroupedCheckboxesGroup[] = [];

  for (const group of groups) {
    if (!isRecord(group) || typeof group.label !== "string" || group.label.length === 0) {
      throw new Error("Grouped checkboxes widget groups require non-empty string labels.");
    }
    if (!Array.isArray(group.values)) {
      throw new Error(`Grouped checkboxes group '${group.label}' requires a values array.`);
    }

    const values: string[] = [];
    for (const value of group.values) {
      if (typeof value !== "string" || value.length === 0) {
        throw new Error(`Grouped checkboxes group '${group.label}' includes an invalid value.`);
      }
      if (!enumValues.has(value)) {
        throw new Error(`Grouped checkboxes option '${value}' is not defined.`);
      }
      if (groupedValues.has(value)) {
        throw new Error(`Grouped checkboxes option '${value}' appears in multiple groups.`);
      }
  ...
  for (const enumValue of enumValues) {
    if (!groupedValues.has(enumValue)) {
      throw new Error(`Grouped checkboxes option '${enumValue}' is missing from groups.`);
    }
  }
...
function resolveEnumValues(
  enumOptions: WidgetProps<JsonObject, RJSFSchema, SchemaFormContext>["options"]["enumOptions"],
): ReadonlySet<string> {
...
function resolveEnumOptionsByValue(
  enumOptions: WidgetProps<JsonObject, RJSFSchema, SchemaFormContext>["options"]["enumOptions"],
): ReadonlyMap<
  string,
  {
...
> {
  ...
  for (const [index, option] of enumOptions.entries()) {
    if (typeof option.value !== "string") {
      continue;
    }
...
export const SchemaFormWidgets = {
  ...
  SelectWidget,
  CheckboxesWidget,
  GroupedCheckboxesWidget,
  TextareaWidget,
  "affixed-text": AffixedTextWidget,
  checkboxes: CheckboxesWidget,
  "grouped-checkboxes": GroupedCheckboxesWidget,
