...
describe.concurrent("data-plane worker reconcile sandbox bootstrap attachment cleanup", () => {
  it(
    "terminates a stale bootstrap attachment when reconcile observes an already-terminal sandbox",
    async ({ env }) => {
      ...
      try {
        ...
        if (attached.attachment === null) {
          throw new Error("Expected bootstrap attachment before running reconcile workflow.");
        }
