...
function createWorkerRuntimeConfig(input: { gatewayBaseUrl: string }) {
  const loadedConfig = loadDataPlaneWorkerConfig({
    ...
    MISTLE_INTERNAL_AUTH_SHARED_TOKEN: InternalAuthServiceToken,
    MISTLE_SANDBOX_PROVIDER: "docker",
    MISTLE_SANDBOX_DEFAULT_BASE_IMAGE: LocalTestSandboxBaseImageRef,
    MISTLE_SERVICES_DATA_PLANE_GATEWAY_SANDBOX_WS_PUBLIC_URL: `${input.gatewayBaseUrl.replace("http://", "ws://")}/tunnel/sandbox`,
    MISTLE_SERVICES_DATA_PLANE_GATEWAY_SANDBOX_WS_INTERNAL_URL: `${input.gatewayBaseUrl.replace("http://", "ws://")}/tunnel/sandbox`,
    ...
    MISTLE_POSTGRES_DATA_PLANE_POOLED_URL: getDatabaseStack().directUrl,
    MISTLE_WORKFLOW_DATA_PLANE_NAMESPACE_ID: "data-plane-worker-deadlines-it",
    MISTLE_SERVICES_DATA_PLANE_WORKER_WORKFLOW_CONCURRENCY: "1",
    MISTLE_SERVICES_DATA_PLANE_GATEWAY_INTERNAL_URL: input.gatewayBaseUrl,
    MISTLE_SERVICES_CONTROL_PLANE_API_INTERNAL_URL: "http://127.0.0.1:5100",
    MISTLE_SERVICES_TOKENIZER_PROXY_EGRESS_URL: "http://127.0.0.1:5400",
