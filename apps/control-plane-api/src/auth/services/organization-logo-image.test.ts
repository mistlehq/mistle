...
describe("normalizeOrganizationLogoImage", () => {
  it("converts a wide JPEG into a square WebP organization logo without upscaling", async () => {
    ...
    const normalized = await normalizeOrganizationLogoImage({
  ...
  it("rejects unsupported image bytes", async () => {
    await expect(
      normalizeOrganizationLogoImage({
