...
const ENCRYPTED_DEVICE_AUTHORIZATION_PROVIDER_STATE_FORMAT_VERSION = "v1";
...
function deriveDeviceAuthorizationProviderStateEncryptionKey(
  masterEncryptionKeyMaterial: string,
): Buffer {
  return createHash("sha256")
    .update("integration-device-authorization-provider-state", "utf8")
    .update("\0", "utf8")
    .update(masterEncryptionKeyMaterial, "utf8")
    .digest();
}

export function encryptDeviceAuthorizationProviderStateUtf8(input: {
  plaintext: string;
  masterKeyVersion: number;
  masterEncryptionKeyMaterial: string;
}): string {
  const encryptionKey = deriveDeviceAuthorizationProviderStateEncryptionKey(
    input.masterEncryptionKeyMaterial,
  );
  const nonce = randomBytes(AES_GCM_NONCE_BYTE_LENGTH);
  const plaintext = Buffer.from(input.plaintext, "utf8");
  const cipher = createCipheriv("aes-256-gcm", encryptionKey, nonce);
  ...
  try {
    ...
    try {
      return [
        ENCRYPTED_DEVICE_AUTHORIZATION_PROVIDER_STATE_FORMAT_VERSION,
        String(input.masterKeyVersion),
        nonce.toString("base64url"),
        ciphertext.toString("base64url"),
        authTag.toString("base64url"),
...
export function decryptDeviceAuthorizationProviderStateUtf8(input: {
  ciphertext: string;
  masterEncryptionKeys: Record<string, string>;
}): string {
  const [formatVersion, encodedMasterKeyVersion, encodedNonce, encodedCiphertext, encodedAuthTag] =
  ...
  if (
    formatVersion !== ENCRYPTED_DEVICE_AUTHORIZATION_PROVIDER_STATE_FORMAT_VERSION ||
    encodedMasterKeyVersion === undefined ||
    encodedNonce === undefined ||
    encodedCiphertext === undefined ||
    encodedAuthTag === undefined
  ) {
    throw new Error("Encrypted device authorization provider state format is invalid.");
  }

  const masterKeyVersion = Number(encodedMasterKeyVersion);
  if (!Number.isInteger(masterKeyVersion) || masterKeyVersion < 1) {
    throw new Error(
      "Encrypted device authorization provider state contains an invalid master key version.",
    );
  }
  ...
  try {
    ...
    throw new Error("Failed to decrypt device authorization provider state.", { cause: error });
  } finally {
    encryptionKey.fill(0);
    nonce.fill(0);
    ciphertext.fill(0);
