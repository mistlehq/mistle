...
const GitHubResourceKinds: readonly ["repository", "branch", "user"] = [
  "repository",
  "branch",
  "user",
];
...
type ResourceRefreshKind = "repository" | "branch" | "user" | "channel";
...
async function expectWorkflowRunCount(
  ...
  expect(Number.parseInt(count, 10)).toBe(input.expectedCount);
}
