...
const ControlPlaneWorkerServiceName = "@mistle/control-plane-worker";

export function defineTracedControlPlaneWorkflow<Input, Output, RawInput = Input>(
  ...
  return defineWorkflow(spec, async (params) => {
    ...
    return runWithWorkflowTraceContext({
      fn: async () => fn(params),
      serviceName: ControlPlaneWorkerServiceName,
      spanName: spec.name,
