...
export type RecoverTriggerConversationSandboxInput = {
  routeId: string;
  sandboxInstanceId: string;
};

export async function recoverTriggerConversationSandbox(
  deps: {
    db: ControlPlaneDatabase | ControlPlaneTransaction;
  },
  input: RecoverTriggerConversationSandboxInput,
) {
  return deps.db.transaction(async (transaction) => {
    ...
    if (persistedRoute.status === TriggerConversationRouteStatuses.CLOSED) {
      throw new TriggerConversationPersistenceError({
        ...
        message: `TriggerConversation route '${input.routeId}' is closed and cannot recover its sandbox.`,
      });
    }
    ...
    if (persistedTriggerConversation.status === TriggerConversationStatuses.CLOSED) {
      throw new TriggerConversationPersistenceError({
        ...
        message: `TriggerConversation '${persistedRoute.conversationId}' is closed and cannot recover its sandbox.`,
      });
    }
    ...
    if (updatedRoute === undefined) {
      throw new TriggerConversationPersistenceError({
        code: TriggerConversationPersistenceErrorCodes.CONVERSATION_ROUTE_NOT_FOUND,
        message: `TriggerConversation route '${input.routeId}' was not found during recovery update.`,
      });
    }
