...
import { deliverTriggerPayload } from "./deliver-trigger-payload.js";
import {
  connectSandboxAgentConnection,
  sendSandboxAgentMessage,
} from "./sandbox-agent-connection.js";

type DeliverTriggerPayloadInput = Parameters<typeof deliverTriggerPayload>[0];
...
function createDeliverInput(connectionUrl: string): DeliverTriggerPayloadInput {
  return {
    preparedTriggerRun: {
      triggerRunId: "aru_test_001",
      triggerRunCreatedAt: "2026-03-05T00:00:00.000Z",
      triggerId: "atm_test_001",
      conversationId: "cnv_test_001",
      triggerTargetId: "atg_test_001",
...
describe("sandbox agent websocket delivery", () => {
  it("delivers the rendered trigger input and closes the socket by default", async () => {
    const server = await startAgentTestServer("accept");

    try {
      const deliverInput = createDeliverInput(server.url);

      await deliverTriggerPayload(deliverInput);
  ...
  it("surfaces stream.open.error responses from the sandbox agent channel", async () => {
    ...
    try {
      await expect(deliverTriggerPayload(createDeliverInput(server.url))).rejects.toThrow(
