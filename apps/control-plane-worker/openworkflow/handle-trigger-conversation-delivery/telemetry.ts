...
export function createTriggerConversationDeliveryTelemetryAttributes(
  input: TriggerConversationDeliveryTelemetryContext,
...
export async function withTriggerConversationDeliverySpan<T>(
  input: {
    name: string;
    telemetryContext: TriggerConversationDeliveryTelemetryContext;
  },
  fn: (span: Span) => Promise<T>,
): Promise<T> {
  return await TriggerConversationDeliveryTracer.startActiveSpan(input.name, async (span) => {
    span.setAttributes(
      createTriggerConversationDeliveryTelemetryAttributes(input.telemetryContext),
    );
...
export function logTriggerConversationDeliveryEvent(input: {
  eventName: string;
  message: string;
  telemetryContext: TriggerConversationDeliveryTelemetryContext;
  attributes?: Record<string, DeliveryTelemetryAttributeValue>;
