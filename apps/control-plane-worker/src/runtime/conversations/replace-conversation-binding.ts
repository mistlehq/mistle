...
export type ReplaceConversationBindingInput = {
  routeId: string;
...
export async function replaceConversationBinding(
  deps: ConversationPersistenceDependencies,
  input: ReplaceConversationBindingInput,
) {
  return deps.db.transaction(async (transaction) => {
    ...
    if (persistedRoute.status === ConversationRouteStatuses.CLOSED) {
      throw new ConversationPersistenceError({
        ...
        message: `Conversation route '${input.routeId}' is closed and cannot replace binding.`,
      });
    }
    ...
    if (persistedConversation.status === ConversationStatuses.CLOSED) {
      throw new ConversationPersistenceError({
        ...
        message: `Conversation '${persistedConversation.id}' is closed and cannot replace binding.`,
      });
    }
    ...
    if (updatedRoute === undefined) {
      throw new ConversationPersistenceError({
        code: ConversationPersistenceErrorCodes.CONVERSATION_ROUTE_NOT_FOUND,
        message: `Conversation route '${input.routeId}' was not found during replace binding update.`,
      });
    }
