...
describe("handleAutomationRun integration", () => {
  ...
  it(
    ...
    async ({ fixture }) => {
      ...
      const workflowClient = await createTestWorkflowClient({
        databaseUrl: fixture.config.workflow.databaseUrl,
        namespaceId: `${fixture.config.workflow.namespaceId}-handle-automation-run-running`,
      });
      ...
      try {
        ...
        expect(queuedTasks).toHaveLength(1);
        expect(queuedTasks[0]?.status).toBe(ConversationDeliveryTaskStatuses.QUEUED);
  ...
  it(
    ...
    async ({ fixture }) => {
      ...
      try {
        ...
        expect(queuedTasks).toHaveLength(1);
        expect(queuedTasks[0]?.sourceOrderKey).toBe("2026-03-08T08:19:30Z#00000000000000001005");
      } finally {
        await workflowClient.stop();
        await database.stop();
      }
