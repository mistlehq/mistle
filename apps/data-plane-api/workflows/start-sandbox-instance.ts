...
} from "../../data-plane-worker/src/runtime/services/insert-sandbox-instance.js";
import { startSandbox } from "../../data-plane-worker/src/runtime/services/start-sandbox.js";
import { stopSandbox } from "../../data-plane-worker/src/runtime/services/stop-sandbox.js";
import {
  markSandboxInstanceFailed,
  markSandboxInstanceRunning,
} from "../../data-plane-worker/src/runtime/services/update-sandbox-instance-status.js";
import { waitForSandboxTunnelConnectAck } from "../../data-plane-worker/src/runtime/services/wait-for-sandbox-tunnel-connect-ack.js";
...
export type StartSandboxInstanceWorkflowInput = StartSandboxInstanceInput & {
...
>(
  ...
  async ({ input: workflowInput, step }) => {
    ...
    async function markInstanceFailed(input: {
      sandboxInstanceId: string;
      failureCode: string;
    ...
    const ensuredSandboxInstance = await step.run({ name: "ensure-sandbox-instance" }, async () => {
      const persisted = await ensureSandboxInstance(
        {
          db: ctx.db,
          provider: ctx.runtimeConfig.sandbox.provider,
        },
        {
      ...
      didSandboxConnectToTunnel = await step.run(
        ...
        async () => {
          return waitForSandboxTunnelConnectAck(
            {
              db: ctx.db,
              policy: ctx.tunnelConnectAckPolicy,
              clock: ctx.clock,
              sleeper: ctx.sleeper,
            },
            {
