...
import { itMemory, type DataPlaneGatewayIntegrationFixture } from "./test-context.js";
...
async function insertSandboxInstanceRow(input: {
  ...
  await input.fixture.db.insert(sandboxInstances).values({
    ...
    organizationId: "org_runtime_state_route_memory_it",
    sandboxProfileId: "sbp_runtime_state_route_memory_it",
    sandboxProfileVersion: 1,
    runtimeProvider: input.fixture.config.sandbox.provider,
    providerRuntimeId: `provider-${input.sandboxInstanceId}`,
    status: SandboxInstanceStatuses.STARTING,
    startedByKind: "system",
    startedById: "workflow_runtime_state_route_memory_it",
    source: "webhook",
  });
}
